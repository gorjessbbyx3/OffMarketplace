...
class HawaiiPropertyScraper {
  ...
  // Scrape BOC Data for additional property records
  async scrapeBOCData() {
    console.log('Scraping BOC Data for property records...');
    const browser = await this.initBrowser();
    const page = await browser.newPage();
    ...
    try {
      ...
      await page.waitForSelector('input[type="text"], #search', { timeout: 10000 });

      // Search for recent property records
      await page.type('#search', 'Honolulu');
      await page.click('input[type="submit"], .search-btn');
      
      await page.waitForSelector('.result-item, .property-record', { timeout: 15000 });

      const properties = await page.evaluate(() => {
        const records = document.querySelectorAll('.result-item, .property-record, tr');
        const results = [];

        records.forEach((record, index) => {
          if (index >= 15) return; // Limit results

          const addressEl = record.querySelector('.address, td:nth-child(1)');
          const ownerEl = record.querySelector('.owner, td:nth-child(2)');
          const valueEl = record.querySelector('.value, td:nth-child(3)');

          if (addressEl && addressEl.textContent.includes('Honolulu')) {
            results.push({
              address: addressEl.textContent.trim(),
              owner: ownerEl?.textContent?.trim() || 'Unknown',
              assessed_value: valueEl?.textContent?.trim() || '0',
      ...
      console.log(`Found ${properties.length} properties from BOC Data`);
      return properties;

    } catch (error) {
      console.error('Error scraping BOC Data:', error);
      return [];
    } finally {
      await page.close();
    }
  ...
  async scrapeAllSources() {
    ...
    try {
      const [foreclosureProperties, oahuProperties, bocProperties] = await Promise.all([
      ...
      const allProperties = [...foreclosureProperties, ...oahuProperties, ...bocProperties].map(property => {
